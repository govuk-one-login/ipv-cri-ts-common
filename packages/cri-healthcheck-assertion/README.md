...
} from "@govuk-one-login/cri-healthcheck-assertion";
...
```ts
await assertFunctionResultWithTimeout(
  "healthcheck",
  5000,
  async () => {
...
This module is currently built to both CJS and ESM standards, so should work universally. However, we expect to move to ESM-only in the future.